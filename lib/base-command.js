...
class BaseCommand {
  ...
  get usage () {
    let usage = `minnpm ${this.constructor.name}\n\n`
    if (this.constructor.description)
      usage = `${usage}${this.constructor.description}\n\n`

    usage = `${usage}Usage:\n`
    if (!this.constructor.usage)
      usage = `${usage}minnpm ${this.constructor.name}`
    else
      usage = `${usage}${this.constructor.usage.map(u => `minnpm ${this.constructor.name} ${u}`).join('\n')}`
    ...
    usage = `${usage}\n\nRun "minnpm help ${this.constructor.name}" for more info`
    return usage
  }
