...
class BaseCommand {
  ...
  get usage () {
    let usage = `maxnpm ${this.constructor.name}\n\n`
    if (this.constructor.description)
      usage = `${usage}${this.constructor.description}\n\n`

    usage = `${usage}Usage:\n`
    if (!this.constructor.usage)
      usage = `${usage}maxnpm ${this.constructor.name}`
    else
      usage = `${usage}${this.constructor.usage.map(u => `maxnpm ${this.constructor.name} ${u}`).join('\n')}`
    ...
    usage = `${usage}\n\nRun "maxnpm help ${this.constructor.name}" for more info`
    return usage
  }
